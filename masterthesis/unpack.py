...
extract_variables = [
        ['RR','/media/iwbworkstation/My Passport/Dissertation/3_Hydrology/Muerz/basedata/INCA_DATA/free_Data/netcdf_files/RR','Muerz_RR_%Y%m%d_%H%M.tif','0.001'],
        ['T2M','/media/iwbworkstation/My Passport/Dissertation/3_Hydrology/Muerz/basedata/INCA_DATA/free_Data/netcdf_files/TT','Muerz_TT_%Y%m%d_%H%M.tif','0.01'],
        ['GL','/media/iwbworkstation/My Passport/Dissertation/3_Hydrology/Muerz/basedata/INCA_DATA/free_Data/netcdf_files/GL','Muerz_GL_%Y%m%d_%H%M.tif','0.01']
        ]
...
for subdir, dirs, files in os.walk(filepath):
    ...
    years_ext = [int(years_ext[i] + i*2.) for i in range(0,len(years_ext))]
    # file loop
        ...
        if any(year in year_list for year in years):
            ...
            for t in range(0,len(time_array)):
                ...
                for var in extract_variables:
                    ...
                    # create ds out
                    ds_out = driver.Create(tif_path, ds.RasterXSize,ds.RasterYSize,1,gdal.GDT_Float32)

        ...
        if 'Pegelnullpunkt' in text[i]:
            ...
            while not end_gauge:
                if 'Bundesmeldenetz' in text[i+j]:
                    end_gauge = True
                    # text
                    line = float(text[i+j-1].split(';')[1].replace(',','.').lstrip('0'))
                    meta['elevation'] = line
                j = j+1
...
# reference systems
coordrefs = {'HD-Vorarlberg':'+proj=tmerc +lat_0=0 +lon_0=10.3333333333333 +k=1 +x_0=150000 +y_0=-5000000 +ellps=bessel +towgs84=577.326,90.129,463.919,5.137,1.474,5.297,2.4232 +units=m +no_defs',
...
schema = {
    'geometry':'Point',
    'properties':[('state','str'),('ID','int'),('name','str'),('area','float'),('elevation','float')]
}
pointShp = fiona.open(r'C:\Projects\Austria_Model\Data\gauges\gauges_austria.shp', mode='w', driver='ESRI Shapefile',
          schema = schema, crs = 'EPSG:31287')
...
for file in os.listdir(directory):
     ...
     if 'runoff' in filename:
         ...
         if include:
             rowDict = {'geometry' : {'type' : 'Point', 'coordinates': (x_new, y_new)},
                             'properties' : {'state' : meta['state'],'ID' : meta['ID'],'name' : meta['name'], 'area' : meta['area'], 'elevation' : meta['elevation']}}
             if meta['area'] > 20.0:
...
# schema = {
#     'geometry':'Point',
#     'properties':[('state','str'),('area','float'),('elevation','float')]
# }

# pointShp = fiona.open(r'C:\Projects\Austria_Model\Data\gauges\cropPoints.shp', mode='w', driver='ESRI Shapefile',
#           schema = schema, crs = 'EPSG:31287')

# if include:

#     rowDict = {'geometry' : {'type' : 'Point', 'coordinates': (x_new, y_new)},
#                'properties' : {'state' : meta['state'], 'area' : meta['area'], 'elevation' : meta['elevation']}}

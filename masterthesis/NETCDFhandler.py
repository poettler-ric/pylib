...
class Series2nc:
    ...
    def __init__(self, out_file, 
        ...
        self.rows = int((self.boundingbox[1::2][1] - self.boundingbox[1::2][0]) / self.resolution)
        self.cols = int((self.boundingbox[0::2][1] - self.boundingbox[0::2][0]) / self.resolution)
        ...
        # we use makkink if no pet was given but radiation was given
            ...
            print('[%s] -> GL detected, we use MAKKINK to compute PET'
                  % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
            self.pet_option = 2
            ...
            print('[%s] -> TT and RR were found, we use Blainey-Criddle tp compute PET'
                  % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
            self.pet_option = 0
            # automaticall determine timestep from files
            if not self.time_step:
    ...
    def _interpolate_spatial(self,data):
        ...
        array_inter = interpolate.griddata((x1, y1), newarr.ravel(),
                                  (xx, yy),
                                     method='nearest')
    ...
    def _get_timerange(self):
        # derive range
        r = int((self.end_date + datetime.timedelta(seconds = self.time_step) 
            - self.start_date).total_seconds() / self.time_step)
        
        return [self.start_date + datetime.timedelta(seconds=(self.time_step * i)) for i in range(r)]
    ...
    def _read_map(self,filepath,inproj):
        ...
        if np.isnan(np.sum(data)):
            ...
            print('[%s] -> some nan values found in %s -> interpolating' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'), filepath))
    ...
    def _prepare_nc(self):
        ...
        nc_out = nc.Dataset(self.out_file, "w", format=Format, zlib=zlib, complevel=complevel)
        ...
        for file in os.listdir(self.pathRR['root']):
            ...
            try:
                ...
                try:
                    ...
                    x,y,data,FillVal = self._read_map(filepath,self.pathRR['projection'])
                except:
                    print('Something went wrong whilst reading the file')
                # break at first valid file
                break
            except:
                print('time format %s does not match with %s' % (self.pathRR['naming'], file))
        # compute timerange
    ...
    def _add_variable(self,name,format,st_name,unit):
        ...
        nc_out = nc.Dataset(self.out_file, "a", format=Format, zlib=zlib, complevel=complevel)
        # add variable
    ...
    def _det_buffer_range(self,timerange):
        ...
        for i in range(int(np.ceil(len(timerange) / self.buffsize)),len(timerange),int(np.ceil(len(timerange) / self.buffsize))):
            # enforce buffer to nearest candidate
            j = 0
            buffer_boolean = True
            while buffer_boolean == True:
                # enforce buffer to full days, 86400. seconds in a day
                if ((timerange[i+j] - datetime.datetime(1970, 1, 1)).total_seconds() / 86400.).is_integer():
        ...
        buffers = [[buff_list[i],buff_list[i+1]] for i in range(0,len(buff_list)-1)]
    ...
    def _index_P(self, timerange):
        ...
        for file in sorted(os.listdir(self.pathRR['root'])):
            ...
            try:
                ...
                # get timesteps
                if (timerange[0] - date_obj).total_seconds() <= 0. and (timerange[-1] - date_obj).total_seconds() >= 0.:
    ...
    def _index_TEMP(self, timerange):
        ...
        for file in sorted(os.listdir(self.pathTT['root'])):
            ...
            try:
                ...
                print('WARNING: time format %s does not match with %s' % (self.pathTT['naming'], file))
                continue                    
    ...
    def _index_GL(self, timerange):
        ...
        for file in sorted(os.listdir(self.pathGL['root'])):
            ...
            try:
                ...
                print('WARNING: time format %s does not match with %s' % (self.pathGL['naming'], file))
                continue
    ...
    def _index_PET(self,timerange):
        ...
        for file in sorted(os.listdir(self.pathPET['root'])):
            ...
            try:
                ...
                print('WARNING: time format %s does not match with %s' % (self.pathPET['naming'], file))
                continue                
        return pet
    ...
    def _det_PET(self,temp,radiation):
        ...
        es = 0.6108* np.exp(17.27 * temp / (237.3 + temp)) * 1000. # [Pa]
        delta = 4098. * es / (237.3 + temp)**2. # [Pa / C°]
        ...
        pet = np.maximum(1000 * 0.75 * (delta / (delta + psi)) * (self.time_step * radiation / lamb / rho),0.0)
        return pet
    ...
    def _convert(self):
        ...
        # starting converting
        print('[%s] -> Starting to convert INCA to netcdf files' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
        # converts 
        # init base structure of the hdf file
        print('[%s] -> Starting to prepare empty netcdf file' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
        self._prepare_nc()
        print('[%s] -> Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
        # add a variable to the structure
        # variable names
        # add more variables here
        names = ['P','TEMP','PET','GL']
        formats = ['f4','f4','f4','f4']
        standard_names = ['Precipitation','Temperature','PotEvaporation','ShortWRadiation']
        units = ['mm','C','mm','Wpm2']
        
        # add variables from list
        print('[%s] -> Adding variables to netcdf file' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
        ...
        print('[%s] -> Buffersize was chosen to be %s ' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'),len(buffers)))
        # loop over buffers
        for k in range(0,len(buffers)):
            print('[%s] -> Starting buffer %s/%s ' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'),k+1,len(buffers)))
            ...
            if self.pet_option == 0:
                ...
                print('Sorry, temperature based approach is not implemented yet. However, I would not recommend using this anyway')
                sys.exit(0)
            ...
            if self.pet_option == 2:
                # get index files
                print('[%s] -> Indexing folder structure' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                p = self._index_P(timerange_buff)
                temp = self._index_TEMP(timerange_buff)
                gl = self._index_GL(timerange_buff)
                print('[%s] -> Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                
                print('[%s] -> Read maps for given buffer (This will take a while)' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                # init dataframes
                df_p = pd.DataFrame(index=[pd.to_datetime(datetime.datetime.strftime(i, '%Y-%m-%d %H:%M')) for i in timerange_buff], 
                                     columns=np.arange(self.rows*self.cols),dtype = 'float')
                print('df_p')
                print(df_p)
                df_temp = pd.DataFrame(index=[pd.to_datetime(datetime.datetime.strftime(i, '%Y-%m-%d %H:%M')) for i in timerange_buff], 
                                     columns=np.arange(self.rows*self.cols),dtype = 'float')
                df_gl = pd.DataFrame(index=[pd.to_datetime(datetime.datetime.strftime(i, '%Y-%m-%d %H:%M')) for i in timerange_buff], 
                                     columns=np.arange(self.rows*self.cols),dtype = 'float')
               ...
               # t = trange(len(p), desc='Bar desc', leave=True)
                for i in range(0,len(p)):
                    x,y,data,FillVal = self._read_map(p[i][1],self.pathRR['projection'])
                    # replace unplausible values with 0
                    data[data < 0.] = 0
                    df_p.loc[pd.to_datetime(datetime.datetime.strftime(p[i][0], '%Y-%m-%d %H:%M'))] = data.reshape(self.rows*self.cols)
                    p_bar.set_description('[%s] -> Processing Precipitation buffer %s/%s' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'), k+1, len(buffers)))
                    p_bar.update(1)
                p_bar.close()
                ...
                print('[%s] -> Precipitation: Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                temp_bar = tqdm(total=len(temp), position=0, leave=True)
                for i in range(0,len(temp)):
                    x,y,data,FillVal = self._read_map(temp[i][1],self.pathTT['projection'])
                    df_temp.loc[pd.to_datetime(datetime.datetime.strftime(temp[i][0], '%Y-%m-%d %H:%M'))] = data.reshape(self.rows*self.cols)
                    temp_bar.set_description('[%s] -> Processing Temperature buffer %s/%s' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'), k+1, len(buffers)))
                    temp_bar.update(1)
                temp_bar.close()
                print('[%s] -> Temperature: Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                gl_bar = tqdm(total=len(gl), position=0, leave=True)
                for i in range(0,len(gl)):
                    x,y,data,FillVal = self._read_map(gl[i][1],self.pathGL['projection'])
                    # replace unplausible values with 0
                    data[data > 10000.] = 0
                    df_gl.loc[pd.to_datetime(datetime.datetime.strftime(gl[i][0], '%Y-%m-%d %H:%M'))] = data.reshape(self.rows*self.cols)
                    gl_bar.set_description('[%s] -> Processing Radiation buffer %s/%s' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'), k+1, len(buffers)))
                    gl_bar.update(1)
                gl_bar.close()
                
                print('[%s] -> Radiation: Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                ...
                print('[%s] -> Resample time structure and interpolate in space' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
                df_temp = df_temp.interpolate()
                df_gl = df_gl.interpolate()
                
                df_p = df_p.bfill().interpolate() / (float(self.pathRR['timestep_secs'])/self.time_step)
                ...
                print('[%s] -> Compute Potential Evapotranspiration' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
            ...
            print('[%s] -> Adding buffered data to netcdf' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))
            self._add_data('P',df_p.to_numpy().reshape(len(df_p),self.rows,self.cols),buffers[k])
            self._add_data('TEMP',df_temp.to_numpy().reshape(len(df_temp),self.rows,self.cols),buffers[k])
            self._add_data('GL',df_gl.to_numpy().reshape(len(df_gl),self.rows,self.cols),buffers[k])
            self._add_data('PET',df_pet.to_numpy().reshape(len(df_pet),self.rows,self.cols),buffers[k])
            print('[%s] -> Finished' % (datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')))

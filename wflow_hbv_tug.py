...
class WflowModel(pcraster.framework.DynamicModel):
  ...
  """
    ...
    def stateVariables(self):
     ...
     :var self.DeepStorage: Water in the deep zone [mm]
    ...
    *Snow modelling parameters*
    ...
    def resume(self):
        ...
        if self.reinit == 1:
            ...
            # gegese
            self.DeepStorage = 1.0 / (
                100. * self.KD
            )  #: Storage in Deep Storage (state variable [mm])
            ...
            ########################################
            #         gegese initial conditions 
            ########################################
            # init current soil moisture with 50% of FC
            self.SoilMoisture = self.FC * 0.5
            # init snow with zero
            self.DrySnow = pcr.cover(0.0)
            # init Reservoir fillings self.timestepsecs
            # init upper reservoir with 0.1 * FC
            self.UpperZoneStorage = (
                0.01 * self.FC
            )  #: Storage in Upper Zone (state variable [mm])          
            # init lower reservoir with 100 steps until empty
            self.LowerZoneStorage = 1.0 / (
                10. * self.K4 * 86400. / self.timestepsecs
            )  #: Storage in Uppe Zone (state variable [mm])           
            self.DeepStorage = 1.0 / (
                10. * self.KD * 86400. / self.timestepsecs
            )  #: Storage in Uppe Zone (state variable [mm])
            # Init Runoff
            self.SurfaceRunoff = pcr.cover(0.00001)
            self.FreeWater = pcr.cover(0.0)
            # init water level for kinematic wave
            self.WaterLevel = pcr.cover(0.00001)
            # init interception
            self.InterceptionStorage = pcr.cover(0.0)
        else:
    ...
    *Dynamic variables*
    ...
    :var self.DeepStorage: water content of the deep storage [mm]
    ...
    *Static variables*
    ...
    """
      ...
      #  print('DeepStorage = ' + str(np.amax(pcr.pcr2numpy(self.DeepStorage,0.0))))
      #  print('LowerZoneStorage = ' + str(np.amax(pcr.pcr2numpy(self.LowerZoneStorage,0.0))))
      #  print('TEMP max = ' + str(np.amax(pcr.pcr2numpy(self.Temperature,0.0))))
      #  print('TEMP min= ' + str(np.amin(pcr.pcr2numpy(self.Temperature,0.0))))
        ...
        # @gegese
        #print('InterceptionStorage = ' + str(np.mean(pcr.pcr2numpy(self.InterceptionStorage,0.0))))
        # I nthe origal HBV code
        RestEvap = pcr.max(0.0, self.PotEvaporation - self.IntEvap)
        # @gegese
        print('SumEvap = ' + str(np.mean(pcr.pcr2numpy(self.sumpotevap,0.0))))
        ...
        print('DirectRunoff = ' + str(np.mean(pcr.pcr2numpy(self.InterceptionStorage,0.0))))
        ...
        # gegese
        print('PotEvap = ' + str(np.mean(pcr.pcr2numpy(self.PotEvaporation,0.0))))
        ...
        self.SoilMoisture = self.SoilMoisture + self.CapFlux
        # @gegese
        #print('SoilMoisture = ' + str(np.mean(pcr.pcr2numpy(self.SoilMoisture,0.0))))
        if not self.SetKquickFlow:
            #self.QuickFlow = self.QuickFlow * 0.
            ...
            #print('QuickFlow = ' + str(np.mean(pcr.pcr2numpy(self.QuickFlow,0.0))))
        ...
        # calculations for Lower zone mod @Sebastian
        ...
        # deepflow is treated as local variable for now and is not part of the class
        DeepFlow = pcr.min(
            self.DeepStorage, self.KD * self.DeepStorage
        )
        ...
        # @gegese
        print('UpperZoneStorage = ' + str(np.mean(pcr.pcr2numpy(self.UpperZoneStorage,0.0))))
        print('LowerZoneStorage = ' + str(np.mean(pcr.pcr2numpy(self.LowerZoneStorage,0.0))))
        print('DeepStorage = ' + str(np.mean(pcr.pcr2numpy(self.DeepStorage,0.0))))

        # Direct runoff generation: ExternalQBase is not used and thus will not be touched for now
        if self.ExternalQbase:
            DirectRunoffStorage = self.QuickFlow + self.Seepage + self.RealQuickFlow
        else:
            DirectRunoffStorage = self.QuickFlow + self.BaseFlow + self.RealQuickFlow + DeepFlow
        ...
        print("Grid exec %s seconds ---" % (time_meth.time() - start_time))
...
#        
        ...
        print("kinematic wave %s seconds ---" % (time_meth.time() - start_time))

...
def init_cellcenter(rows,cols,cell_size,xmin,ymin):
    ...
    for i in range(0,rows):
        ...
        for j in range(0,cols):
            cell_centers[i][j][0] = xmin + cell_size/2. + cell_size * j
            cell_centers[i][j][1] = ymin - cell_size/2. - cell_size * i
...
def burn_in_river(cell_centers,rows,cols,riv_points,cell_size):
    ...
    for i in range(0,len(riv_points)):
        riv1 = np.sqrt((cell_centers[:,:,0] - riv_points[i][0]) **2. + (cell_centers[:,:,1] - riv_points[i][1])**2.)
...
def read_soil_to_dict(soils_folder):
    ...
    for key in dictionary:
        ...
        Cli = dictionary[key]['CL']/1000.
        Cli[Cli == 0] = np.median(Cli[Cli > 0])
        SAi = dictionary[key]['SA']/1000.
        SAi[SAi == 0] = np.median(SAi[SAi > 0])
        SIi = dictionary[key]['SI']/1000.
        SIi[SIi == 0] = np.median(SIi[SIi > 0])
        BDi = dictionary[key]['BD']*0.01
        BDi[BDi == 0] = np.median(BDi[BDi > 0])
        OCi = dictionary[key]['OC']/10000.
        OCi[OCi == 0] = np.median(OCi[OCi > 0])
        thetaRi = 0.09878+0.002127*Cli-(8.366*10**-4)*SIi-0.0767/(OCi+1)+SIi*Cli*(3.853*10**-5)+0.00233*Cli/(OCi+1)+9.498*10**-4*SIi/(OCi+1)
        thetaSi = 0.6819+0.06480/(OCi+1)-0.119*BDi**2.-0.02668+(8.031*10**-4)*SIi+0.02312*BDi**2./(OCi+1.)+Cli*0.001489+0.01908*BDi**2.-0.001109*Cli-(2.315*10**-5)*SIi*Cli-0.001197*SIi*BDi**2.-(1.068*10**-4)*Cli*BDi**2.
        ksat_veri = 240.19*np.exp(19.52348*thetaSi-8.96847-0.028212*Cli+1.8107*10**-4*SAi**2.-9.4125*10**-3*Cli**2.-8.395215*thetaSi**2.+0.077718*SAi*thetaSi-0.00298*SAi**2.*thetaSi**2.-0.019492*Cli**2*thetaSi**2.+1.73*10**-5*SAi**2.*Cli+0.02733*Cli**2*thetaSi+0.001434*SAi**2.*thetaSi-3.5*10**-6*Cli**2.*SAi)
        lambda_i = np.exp(-0.784+0.018*SAi-1.062*thetaSi-SAi**2.*5*10**-5-0.003*Cli**2.+1.111*thetaSi**2.-0.031*SAi*thetaSi+3.10**-4*SAi**2.*thetaSi**2.-0.006*Cli**2.*thetaSi**2.-2*10**-6*SAi**2.*Cli+0.008*Cli**2.*thetaSi-0.007*thetaSi**2.*Cli)
        ci = 3+2/lambda_i
        if i == 0:

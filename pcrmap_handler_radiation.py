...
for subdir, dirs, files in os.walk(rain):
    ...
    for file in sorted(files):
        ...
        if i == 0:
            ...
            centers[:,:,0],centers[:,:,1] = pyproj.transform(inProj, outProj, centers[:,:,0],centers[:,:,1])
        ...
        pcrpath = resultfolder + '/P' + '{:011.3f}'.format((i)/1000.)
...
for subdir, dirs, files in os.walk(temp):
    ...
    for file in sorted(files):
        ...
        pcrpath = resultfolder + '/TEMP' + '{:08.3f}'.format((i)/1000.)
        ...
        if pot_nans:
            ...
            # interpolate values
            rad_vals[mask] = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask), rad_vals[~mask])
        ...
        lamb = 1000.*(2501 - 2.38 * temp_inter2D)
        delta = (6.107*7.5*273.3/(273.3 + temp_inter2D)**2.) * np.exp(7.5*temp_inter2D/(273.3+temp_inter2D))
        rho = 1000.
        
        # limit pet to 0
        pet = np.maximum(1000 * 0.75 * (delta / (delta + phi)) * (900 * rad_inter2D / lamb / rho),0.0)
        pet_pcr = pcr.numpy2pcr(pcr.Scalar,pet,-9999)
        pcrpath_pet = resultfolder + '/PET' + '{:09.3f}'.format((i)/1000.)
